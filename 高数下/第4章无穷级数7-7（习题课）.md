...
## （1）中南大学开放式精品示范课堂高等数学建设组

1．数项级数 $\left\{\begin{array}{l}\text { 正项级数 } \\ \text { 交错级数 } \\ \text { 任意项级数 }\end{array}\right.$
2．幂级数 $\left\{\begin{array}{l}\text { 幂级数的收敛半径与收敛域 } \\ \text { 幂级数的和函数与数项级数的和 } \\ \text { 将函数展开成Taylor级数 }\end{array}\right.$
3．Fourier级数 $\left\{\begin{array}{l}\text { 周期函数展开成Fourier级数 } \\ {[0, \pi] \text { 上的函数展开成正（余）弦级数 }} \\ \text { Fourier级数的和函数 }\end{array}\right.$
