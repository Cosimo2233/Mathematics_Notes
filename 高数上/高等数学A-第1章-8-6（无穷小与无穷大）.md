...
\begin{tabular}{|l|l|}
\hline \multirow{2}{*}{无穷少 无 穷大} & \\
\hline & \begin{tabular}{l}
1．6．1 无穷小 $\left\{\begin{array}{l}\text { 无穷小的定义 } \\ \text { 无穷小与函数极限的关系 }\end{array}\right.$ \\
1．6．2 无穷大 $\left\{\begin{array}{l}\text { 无穷大的定义 } \\ \text { 无穷大与无穷小的关系 }\end{array}\right.$ \\
1．6．3 无穷小与无穷大的运算 $\left\{\begin{array}{l}\text { 有限个无穷小的代数运算 } \\ \text { 有界函数与无穷小的乘积 } \\ \text { 有限个无穷小的乘积 } \\ \text { 无穷大的简单运算 }\end{array}\right.$ \\
1．6．4 无穷小的比较 $\left\{\begin{array}{l}\text { 无穷小阶的定义 } \\ \text { 等价无穷小及性质 }\end{array}\right.$ \\
习例1－12
\end{tabular} \\
\hline
\end{tabular}
...
\section*{2．无穷小与函数极限的关系}

\section*{（以下定理中＂ $\lim "$ 表示 $x \rightarrow x_{0}$ 或 $x \rightarrow \infty$ 等都成立）}
...
$\exists \delta_{2}>0$ ，当 $0<\left|x-x_{0}\right|<\delta_{2}$ 时，有 $\left\lvert\, \beta(x)<\frac{\varepsilon}{2}\right.$ ．
...
则 $|u(x) \cdot \alpha(x)|=|u(x)| \cdot \alpha(x) \left\lvert\,<M \cdot \frac{\varepsilon}{M}=\varepsilon\right.$ 。
...
\section*{定理5 对于自变量相同变化趋势下的无穷大有如下性质：}
（1）有限个无穷大的乘积是无穷大；
（2）无穷大与有界量之和是无穷大．
证（1）设 $\lim _{x \rightarrow x_{0}} f(x)=\infty, ~ \lim _{x \rightarrow x_{0}} g(x)=\infty$ ，下证 $\lim _{x \rightarrow x_{0}}[f(x) g(x)]=\infty$ ． $\forall M>0$ ，因 $\lim _{x \rightarrow x_{0}} f(x)=\infty, \exists \mathcal{E}_{1}>0$ 当 $0<\left|x-x_{0}\right|<\delta_{1}$ 时，有 $|f(x)|>\sqrt{M}$ ，
...
![](https://cdn.mathpix.com/cropped/32d5880b-9704-475f-adbb-c993a2e30baf-13.jpg?height=717&width=197&top_left_y=559&top_left_x=75)
...
$$
\begin{aligned}
& \int_{x \rightarrow 0} \frac{a^{n}-b^{n}=(a-b)\left(a^{n-1}+a^{n-2} b+\cdots+b^{n-1}\right)}{\frac{1}{n} x \cdot\left[(\sqrt[n]{1+x})^{n-1}+(\sqrt[n]{1+x})^{n-2}+\cdots+1\right]}=1 \\
& \therefore \lim _{\text {当 }} x \rightarrow 0 \text { 时, } \sqrt[n]{1+x}-1 \sim \frac{1}{n} x
\end{aligned}
$$
...
\section*{例3 证明当 $x \rightarrow 0$ 时，$e^{x}-1 \sim x$}
